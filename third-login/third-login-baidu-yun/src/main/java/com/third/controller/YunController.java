...
public class YunController {
	...
	@RequestMapping("/mine")
	public String mine(ModelMap map, HttpServletRequest req, HttpServletResponse resp) {
		String cookieId=CookieUtils.getCookie(req, resp);
		if ("".equals(cookieId)||cookieId==null) {
			return "redirect:login";
		}
		Object user = storeFactory.get(cookieId);
		if (user == null) {
			return "redirect:/login";
		}
		map.addAttribute("yunUser", (Map<String, Object>) user);
		return "mine";
	}
